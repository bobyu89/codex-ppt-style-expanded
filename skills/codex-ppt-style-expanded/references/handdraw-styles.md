...
使用者想要手繪、手寫、筆記或特定媒材時讀本文件。九種配方以 `HW01–HW09` 或完整 ID 查詢；這是 SlideWeave 自有編號，不對應外部畫廊的 001–261。
...
| HW07 | risograph-duotone | 雙專色疊印、細顆粒與微幅套印偏移 | 文化活動、創意提案 |
| HW08 | cut-paper-collage | 纖維紙邊、不規則剪形、霧面色紙與淺層投影 | 故事教學、社區與人文 |
| HW09 | single-panel-cartoon | 黑色細墨線、大量白底、極少灰階與淡色重點 | 演講開場、觀點與概念反思 |
...
字體搭配獨立於 HW 風格，不另計一種風格。可選 `heading_style: clear`（預設）或 `heading_style: handwritten`，正文維持 `body_style: clear`。這些是需求紀錄欄位；製作時須將實際可用的字體寫入原生文字物件，不能只記錄選項。手寫標題也要檢查繁中字形、長標題換行與投影可讀性。引用、數值及流程標籤沿用清楚字體。

例如：「使用 HW08；標題採可編輯手寫字，正文使用清楚的繁中字體。」插畫可以保留剪紙媒材，文字仍獨立修改。
