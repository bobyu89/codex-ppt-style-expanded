...
### 2. 搜尋與收斂風格
讀取 [references/style-expansion.md](references/style-expansion.md)。優先採用使用者明確指定的風格或參考圖；沒有指定時推薦三種，最多五種。每種附配色、圖像語言、密度、適用原因。五種是每輪比較上限，不是風格庫容量上限。

用 `python scripts/search_styles.py --query "醫療 教學 illustration" --limit 5` 查詢；原始資料與衍生配方皆可离線讀取。詞彙搜尋只作檢索，不把分數當作美學判斷。

### 3. 直接製作可比較樣張
有足夠的內容與 1–5 個合理候選後，使用同一份代表性內容、相同比例與大致相同資訊量，為每種候選生成一張無字背景，組裝原生文字，渲染成樣張供討論。不要先額外要求使用者從風格名稱選出唯一一種才能看圖。
...
依每頁角色改變版型，保持同一視覺語言。背景與文字共用座標規劃，先規劃內容區再生圖。單頁修改只重製受影響的素材或文字；不要重生整份簡報。
